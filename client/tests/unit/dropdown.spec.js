...
import { faUser, faBolt, faPencilAlt,faTimes, faSignOutAlt } from '@fortawesome/free-solid-svg-icons'
...
describe('Dropdown.vue', () => {
  it('ensure callback gets fired when a dropdown item gets selected', () => {
    ...
    const wrapper = mount(Dropdown, {
      propsData: {items: [{icon:'sign-out-alt', name: 'Sign Out', onClick:mockFunc}]}
    })
  ...
  it('ensure dropdown is closed when prop specifies it to be closed', () => {
    const mockFunc = jest.fn();
    const wrapper = mount(Dropdown, {
      propsData: {items: [{icon:'sign-out-alt', name: 'Sign Out', onClick:mockFunc }], isOpen: false }
    })
